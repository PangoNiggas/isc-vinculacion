...
<div id="page-content-wrapper" class="form-group" >
    <div class="container-fluid">
        @if (str_contains($fase,"Plan de Negocios Final"))
            <div class="container">
                <h2>Entrega de Plan de Negocios</h2>
                <br>
                <!-- Nav pills -->
                <ul class="nav nav-pills" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link active" data-toggle="pill" href="#home">Entrega Final</a>
                    </li>
                </ul>
                <!-- Tab panes -->
                <div class="tab-content">
                    <div id="home" class="container tab-pane active">
                        ...
                        @endif
                        @if (session()->has('infor4'))
                            <div class="alert alert-success"> {{ session('infor4')}}</div>
                        @endif
                        ...
                        @endif
                        <br>          
                        <form class="form-group" method="post" action="{{ route('Enviar.entregaFinal')}}" enctype="multipart/form-data">
                            {{ csrf_field() }}
                            <p>Subir un archivo PDF con todas las fases anteriormente acreditadas. Recuerda que solo puedes subir el archivo una sola vez</p>
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td colspan="4">
                                            @if (empty($avanceFinal))
                                                <div class="form-group">
                                                    <label for="fileEnvio">Documento</label>
                                                    <input type="file" name="documentoFinal" class="form-control" id="fileFinal" onchange="validarExt1()">
                                                    {!! $errors->first('documentoFinal', '<span class=errores>:message</span>') !!}
                                                </div> 
                                                <button type="submit" class="btn btn-primary">Guardar</button>
                                            @else
                                                <div class="alert alert-success">   
                                                    <p>Ya realizaste la entrega final</p>
                                                </div>
                                            @endif
                                        </td>
                                    </tr>
                    ...
                    <div id="menu1" class="container tab-pane fade">
                        ...
                        <p>Subir un archivo Word con las actividades solicitas en la fase en curso. Recuerda que solo puedes subier el archvio word solo una vez</p>
                            @if (session()->has('infor2'))
                                <div class="alert alert-success">{{ session('infor2')}}</div>
                            @endif
                                <form class="form-group" method="post" action="{{ route('Enviar.entregados')}}" enctype="multipart/form-data">
                                    {{ csrf_field() }}
                                    <table class="table table-bordered">
                                        <thead class="thead-light">
                                            <tr>
  ...
  function validarExt1() {
    var archivoInput = document.getElementById('fileFinal');
    var archivoRuta = archivoInput.value;
    var extPermitidas = /(.pdf|.PDF|.Pdf)$/i;
      ...
      if (!extPermitidas.exec(archivoRuta))
      {
        ...
        Swal.fire({
            type: 'error',
            title: 'Oops...',
            text:'Elije una extención en pdf (.PDF)',
